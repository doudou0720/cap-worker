	...
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		if (origin) {
			...
			if (!ALLOWED_ORIGINS.includes(originDomain as any)) {
				return new Response(
					JSON.stringify({ success: false, error: "Access denied: Origin not allowed" }),
					{
						status: 403,
		...
		if (request.method === "POST" && url.pathname === REDEEM_PATH) {
			...
			if (!token || !solutions || !Array.isArray(solutions)) {
				return new Response(
					JSON.stringify({ success: false, error: "Missing token or solutions" }),
					{
						status: 400,
						headers: {
							"Content-Type": "application/json",
		...
		if (request.method === "POST" && url.pathname === VALIDATE_PATH) {
			...
			try {
				...
				return new Response(JSON.stringify({ success: false, error: "Token already consumed" }), {
					status: 409,
					headers: {
						"Content-Type": "application/json",
						...CORS_HEADERS,
